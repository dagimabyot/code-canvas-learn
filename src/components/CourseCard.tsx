...
const CourseCard: React.FC<CourseCardProps> = ({ course, onClick }) => {
  ...
  return (
    <Card 
      onClick={onClick}
      className="group cursor-pointer bg-white border-slate-200 hover:border-primary/50 transition-all duration-300 overflow-hidden flex flex-col shadow-sm hover:shadow-xl"
    >
      <div className="relative aspect-video overflow-hidden">
        ...
        <div className="absolute bottom-3 right-3 flex items-center gap-1 bg-white/90 backdrop-blur-md px-2 py-1 rounded-md shadow-sm">
          ...
          <span className="text-[10px] font-bold text-slate-900">{course.rating || '4.5'}</span>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 text-slate-400 font-bold">
            <span className="text-xs">{(course.enrolled_count || 0).toLocaleString()} learners</span>
          </div>
          <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-primary group-hover:text-white transition-all shadow-inner">
